...
export class ChartsColumnComponent {
  ...
  topChart = signal<EChartsOption>({});
  bottomChart = signal<EChartsOption>({});
  ...
  private buildSeverityChart(stats: any): EChartsOption {
    ...
    return {
      ...
      xAxis: { type: 'value', axisLine: { lineStyle: { color: '#1e3a5f' } }, splitLine: { lineStyle: { color: '#0d1f36' } } },
      yAxis: { type: 'category', data: levels.map(l => l.toUpperCase()), axisLabel: { color: '#8899aa' } },
      series: [{
        type: 'bar',
  ...
  private buildAttackTypesChart(stats: any): EChartsOption {
    ...
    const data   = types.map((t, i) => ({
      name: t, value: stats.attack_types[t],
  ...
  private buildEpmChart(stats: any): EChartsOption {
    ...
    return {
      ...
      xAxis: { type: 'category', data: buckets.map((b: any) => b.minute), axisLabel: { color: '#8899aa', fontSize: 9 }, axisLine: { lineStyle: { color: '#1e3a5f' } } },
      ...
      series: [{
        ...
        areaStyle: { color: { type: 'linear', x: 0, y: 0, x2: 0, y2: 1, colorStops: [{ offset: 0, color: '#00d4ff44' }, { offset: 1, color: '#00d4ff00' }] } },
        symbol: 'none',
      }],
    };
  }
  ...
  private buildTopIpsChart(stats: any): EChartsOption {
    ...
    return {
      ...
      xAxis: { type: 'value', axisLine: { lineStyle: { color: '#1e3a5f' } }, splitLine: { lineStyle: { color: '#0d1f36' } } },
      yAxis: { type: 'category', data: top.map((x: any) => x.ip), axisLabel: { color: '#8899aa', fontSize: 9 } },
      series: [{
        type: 'bar',
